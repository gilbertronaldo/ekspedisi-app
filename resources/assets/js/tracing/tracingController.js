;(() => {
        ...
        function TracingController(
            ...
            ctrl.code = 1;
            ctrl.detail = {};
            ctrl.canEdit = $rootScope.authCan('TRACING_EDIT');
            ctrl.canDelete = $rootScope.authCan('TRACING_DELETE');
            ctrl.editMode = false;
            ...
            ctrl.onDelete = () => {
                swangular.confirm('Apakah anda yakin ingin hapus data ini', {
                    showCancelButton: true,
                    preConfirm: () => {

                        ctrl.isSaving = true;
                        TracingService.deleteTracing(ctrl.tracing.tracing_id)
                            .then(function (result) {
                                ctrl.isSaving = false;

                                swangular.success("Berhasil Menghapus Tracing");
                                getTracing();
                            })
                            .catch(err => {
                                ctrl.isSaving = false;
                                console.log(err);
                                swangular.alert("Error Tracing");
                            })
                    },
                });
            }
